...
class ShopController extends FontEndController {
    public function index(){
        ...
        $order=$_GET['order'];
        if(empty($order)){ 
            $list=$goodsmodel->where("shop_id='$shop_id' and is_delete=0")->order('daijinquan desc,buy_number desc,score desc,last_update desc')->select();
        }elseif($order==='number_desc'){
            $list=$goodsmodel->where("shop_id='$shop_id' and is_delete=0")->order('buy_number desc,last_update desc')->select();
        }elseif($order==='price_desc'){
            $list=$goodsmodel->where("shop_id='$shop_id' and is_delete=0")->order('tuan_price desc,last_update desc')->select();
        }elseif($order==='pinglun_desc'){
            $list=$goodsmodel->where("shop_id='$shop_id' and is_delete=0")->order('score desc,last_update desc')->select();
        }elseif($order==='update_desc'){
            $list=$goodsmodel->where("shop_id='$shop_id' and is_delete=0")->order('last_update desc')->select();
        }elseif($order==='price_asc'){
            $list=$goodsmodel->where("shop_id='$shop_id' and is_delete=0")->order('tuan_price,last_update desc')->select();
        }
         $this->get_thumb($list);
