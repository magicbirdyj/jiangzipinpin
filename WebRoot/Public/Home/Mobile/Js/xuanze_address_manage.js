...
function onreadyeditAddress(){
   ...
   $('body').on('click','.edit_left',function(){
    ...
    function click_shiyong(obj){
        //if(obj.children('.address_right').html()=='使用'){
            //shezhi_zhengyong(obj);
        //}
        shezhi_zhengyong(obj);
        obj.children('.address_right').css('color','#FFF');
        obj.children('.address_right').html('正用');
        obj.children('.address_right').css('background-color','#F90505');
        obj.parents('li').siblings('li').find('.address_right').css('background-color','#FFF');
        obj.parents('li').siblings('li').find('.address_right').css('color','#666');
        obj.parents('li').siblings('li').find('.address_right').html('使用');
    ...
    function shezhi_zhengyong(obj){
       ...
       $('#address_name').html(name);
       $('#address_mobile').html(tel);
       $('#address_location').html(location);
       $('#address_address').html(address);
    }
    ...
    function shezhi_moren(id){
        ...
        var url='/Home/Ajaxlogin/shezhi_moren_address';
        $.ajax({
            type:'post',
            async : true,
            url:url,
    ...
    function save_or_add_address(data){
        var url='/Home/Ajaxlogin/save_or_add_address';
        $.ajax({
            type:'post',
            async : true,
            url:url,
            ...
            success:function(){
                ...
                //如果是添加第一个地址，自动把第一个地址设置为使用。
                    if($('.address_ul>li').length==1){
                        click_shiyong($('.shezhi_zhengyong'));
                    }
                //清空编辑器
                clear_edit();
            }
    ...
    function del(id){
        ...
        var url='/Home/Ajaxlogin/delete_address';
        $.ajax({
            type:'post',
            async : true,
            url:url,
    ...
    $('#address_shoudong').bind('click',function(){
        if($('.address_ul>li').length>=6){
            ...
            $('.big-shade-all').show();
            $('#edit_address_div').show();
            save_or_add='add';
            
            //自动选择城市到湖南省娄底市娄星区
            $("select[name='address_province']").find("option").eq(19).attr("selected","selected");
            province_onchange(19);
            $("select[name='address_city']").find("option").eq(12).attr("selected","selected");
            city_onchange(12);
        }
    });
    
    //关闭添加地址
    $('#tb_close').bind('click',function(){
        $('.big-shade-all').hide();
        $('#edit_address_div').hide();
    });
